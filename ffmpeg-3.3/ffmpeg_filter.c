...
static void init_input_filter(FilterGraph *fg, AVFilterInOut *in)
{
    ...
    enum AVMediaTypeFFMPEG type = avfilter_pad_get_type(in->filter_ctx->input_pads, in->pad_idx);
    int i;
    ...
    if (in->name) {
        ...
        for (i = 0; i < s->nb_streams; i++) {
            enum AVMediaTypeFFMPEG stream_type = s->streams[i]->codecpar->codec_type;
            if (stream_type != type &&
                       ...
                       const char *filter_name)
{
    ...
    AVFilterContext *ctx;
    const AVFilter *trim;
    enum AVMediaTypeFFMPEG type = avfilter_pad_get_type((*last_filter)->output_pads, *pad_idx);
